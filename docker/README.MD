...
El docker compose también me permite utilizar las herramientas de logs, puedo apuntar a un servicio en específico del compose 
...
[+] Building 1.1s (11/11) FINISHED                                                                                                                                                                                                                                                                                       
 ...
 => [5/5] COPY [., /usr/src/]                                                                                                                                                                                                                                                                                       0.1s
 => exporting to image                                                                                                                                                                                                                                                                                              0.1s
 => => exporting layers                                                                                                                                                                                                                                                                                             0.1s
 => => writing image sha256:bc214885f1c17759148882ef08b48c9753608fccf725037f63acfece545699bd                                                                                                                                                                                                                        0.0s
 => => naming to docker.io/library/docker-app                                                                                                                                                                                                                                                                       0.0s
[+] Running 3/3
 ⠿ Network docker_default  Created                                                                                                                                                                                                                                                                                  0.1s
 ⠿ Container docker-db-1   Started                                                                                                                                                                                                                                                                                  0.6s
 ⠿ Container platziapp2    Started    
```
Y puedo ver como mi compose se encuentra activo:
```bash
docker compose ps
```
Respuesta esperada:
...
platziapp2          docker-app          "docker-entrypoint.s…"   app                 About a minute ago   Up About a minute   0.0.0.0:3000->3000/tcp, :::3000->3000/tcp
```

FELICIDADES. Ahora podemos cambiar los archivos en mi computadora local y se verá reflejado en los archivos del compose y adicionalmente
el servidor se recargará automáticamente permitiéndome ver los cambios de inmediato.
...
3. Configurar el anterior archivo base como estaba e introducir lo siguiente:
   - Los puertos (port) y las dependencias (depend_on) deberían ir en el archivo base y el archivo override los tomará de allí
   - Si declaramos variables de entorno (environment) tanto en nuestro archivo base (docker-compose.yml) y en nuestro archivo override (docker-compose.override.yml), lo que hará docker compose es que tomará las dos variables de entornos y las creara.
...
4. Luego levantar los servicios
5. Realizar la prueba en el navegador

Ejecución de los pasos para configurar un archivo.override.yml con el ejemplo anterior

Regresando el `docker-compose.yml` a su estado base:
...
services:
  ...
  db:
    image: mongo
```
Creemos nuestro archivo `override`:

Una versión ligeramente modificada que cumpla con nuestros objetivos particulares de la sesión que estamos manejando actualmente.
Por ejemplo si en local a nosotros nos interesa construir nuestro contenedor desde un Dockerfile en lugar de usar una imagen previamente
generada. 
...
Esta versión sobreescribirá a la primera, añadiendo información y compartiendo la de la base original, en este caso
no necesito en el override escribir los `depends_on` o `ports` e incluso la otra imagen `db` porque como en esta versión
también las voy a utilizar, entonces no tengo necesidad de volver a escribir las cosas que NO están cambiando.

Vuelvo a construir el compose:
```bash
docker compose up -d
```
...
[+] Running 3/3
 ...
 ⠿ Container platziapp2    Recreated                                                                                                                                                                                                                                                                                0.1s
 ⠿ Container docker-app-1  Started  
```

Para estar seguros de que el compose se hizo build desde el override podemos inspeccionar a la aplicación:

```bash
docker inspect --format '{{.Config.Env}}' docker-app-1
```
Respuesta esperada:
```commandline
[MONGO_URL=mongodb://db:27017/test UNA_VARIABLE=HOLA MUNDO! PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin NODE_VERSION=12.22.12 YARN_VERSION=1.22.18]
```
Y vemos como tiene un variable de entorno llamada `UNA_VARIABLE` algo que SOLO está presente en el override.

De esta manera `override` me permite trabajar más cómodamente en equipo, puesto que puedo añadir este archivo a un .gitignore
y evitar que los entornos de mis compañeros de trabajo se estén modificando constantemente cuando yo altere este archivo.

### Escalando contenedores:

Imaginemos que Quiero crear dos contenedores de este proyecto, sería tan sencillo como:

```bash
docker compose up -d --scale app=2
```
Sin embargo, voy a obtener el siguiente error:
```commandline
[+] Running 2/3
 ⠿ Container docker-db-1   Running                                                                                                                                                                                                                                                                                  0.0s
 ⠿ Container docker-app-2  Started                                                                                                                                                                                                                                                                                  0.8s
 ⠿ Container docker-app-1  Starting                                                                                                                                                                                                                                                                                 1.0s
Error response from daemon: driver failed programming external connectivity on endpoint docker-app-1 (a81fd20671dfc5864d685df44d145f11d934a54ba890b2852fb036cf26c591c5): Bind for 0.0.0.0:3000 failed: port is already allocated
```

Esto se debe a que en mi archivo principal de configuración `docker-compose.yml` solo tengo un puerto disponible en mis puertos locales
que quiero hacer puente con el puerto del contenedor:
...
Entonces cuando docker intenta construir el segundo contenedor este ya no se puede generar porque ya tiene el puerto utilizado, 
esto es tan simple como crear un rango de puertos disponibles de mi local apuntando al del contenedor de la siguiente manera.
...
Ahora detenemos el contenedor pasado y volvemos a construir la versión modificada:
...
[+] Running 4/4
 ...
 ⠿ Container docker-app-2  Removed                                                                                                                                                                                                                                                                                  0.3s
 ⠿ Container docker-db-1   Removed                                                                                                                                                                                                                                                                                  0.3s
 ⠿ Network docker_default  Removed  
 
 [+] Running 4/4
 ⠿ Network docker_default  Created                                                                                                                                                                                                                                                                                  0.1s
 ⠿ Container docker-db-1   Started                                                                                                                                                                                                                                                                                  0.5s
 ⠿ Container docker-app-2  Started                                                                                                                                                                                                                                                                                  0.7s
 ⠿ Container docker-app-1  Started  
```
Y ahora tengo dos contenedores en linea uno en el puerto 3000 y otro en el 3001:
```bash
docker compose ps
```
Respuesta esperada:
```commandline
NAME                IMAGE               COMMAND                  SERVICE             CREATED              STATUS              PORTS
docker-app-1        platziapp           "docker-entrypoint.s…"   app                 About a minute ago   Up About a minute   0.0.0.0:3001->3000/tcp, :::3001->3000/tcp
docker-app-2        platziapp           "docker-entrypoint.s…"   app                 About a minute ago   Up About a minute   0.0.0.0:3000->3000/tcp, :::3000->3000/tcp
docker-db-1         mongo               "docker-entrypoint.s…"   db                  About a minute ago   Up About a minute   27017/tcp
```
Limpiamos todo:
```commandline
...
En esta clase mostraremos los comandos más utilizados para eliminar: 
- imagenes
- contenedores
- volumenes
- networks

Con el fin de tener una limpieza general del sistema.

Elimina todos los contenedores sin usar:
```bash
docker container prune
```
...
```
Versión que elimina todo menos las imagenes:
```bash
docker system prune
```
...
```
Limitar el consumo de RAM en un contenedor
```bash
docker run -d --name <container_name> --mormoy <quantity> <image_name>
```
Ver el consumo de los contenedores
```bash
docker stats
```
...
[Dockerfile](docker_platzi/docker/avanzado/loop/Dockerfile)
```doctest
FROM ubuntu:trusty
COPY ["loop.sh", "/"]
CMD /loop.sh
```
[loop.sh](docker_platzi/docker/avanzado/loop/loop.sh)
```doctest
#!/usr/bin/env bash
trap 'exit 0' SIGTERM
while true; do :; done
...
El primero es el dockerfile; sirve para construir nuestra imagen y el segundo es el archivo bash que se ejecutará como un
main process o proceso principal del contenedor. Es importante destacar que la línea de CMD ejecuta nuestro archivo bash, 
a través de un shell, esta forma de expresar esta línea se conoce como shell form y esto genera que al momento de detener 
un contenedor que esté ejecutando un proceso por shell form necesariamente tendrá que usar un SIGKILL. Para evitar esto, 
el CMD debe ejecutar nuestro main process de form exec form, es decir nuestro archivo debe estar configurado de esta manera.
...
La idea de que el archivo esté configurado de esta manera es que de tiempo a que el contenedor procese las solicitudes 
restantes y pueda hacer un Great full shutdown. Otra nota importante es que los códigos de salida mayor a 128 es el 
resultado de una salida por un código de excepción.

Comandos utilizados en clases

```bash
cd docker/avanzado/loop
```
...
Respuesta esperada:
```commandline
 => [2/2] COPY [loop.sh, /]                                                                                                                                                                                                                                                      0.1s
 => exporting to image                                                                                                                                                                                                                                                           0.1s
 => => exporting layers                                                                                                                                                                                                                                                          0.0s
 => => writing image sha256:6c4eb1a2bf4dd920603ca34f367b94e2a754e7682d39b464856d82e3db9b9ddd                                                                                                                                                                                     0.0s
 => => naming to docker.io/library/loop 
```
...
```commandline
CONTAINER ID   IMAGE     COMMAND                 CREATED          STATUS          PORTS     NAMES
380cd7af2c7b   loop      "/bin/sh -c /loop.sh"   26 seconds ago   Up 25 seconds             looper
```
Lo detenemos de forma normal:
```bash
docker stop looper
docker ps -l
```
Esto demora un rato en denerese hasta que por fin lo termina de ejecutar. 
```commandline
CONTAINER ID   IMAGE     COMMAND                 CREATED              STATUS                        PORTS     NAMES
380cd7af2c7b   loop      "/bin/sh -c /loop.sh"   About a minute ago   Exited (137) 25 seconds ago             looper
```
Observamos como su código de `exited` fue el 137, cualquier código encima del 128 es un código que indica que se tuvo que forzar
la salida son sigkill

Borramos el contenedor, lo volvemos a crear pero lo detenemos ahora con `kill`
...
```commandline
CONTAINER ID   IMAGE     COMMAND                 CREATED         STATUS                       PORTS     NAMES
5dc9f0b0f6e1   loop      "/bin/sh -c /loop.sh"   9 seconds ago   Exited (137) 4 seconds ago             looper
```
Esta vez fue mucho más rapido porque directamente envio la señal de `sigkill`

Observemos más detalles de como está corriendo el contenedor:
```bash
docker rm looper
docker run -d --name looper loop
...
root           1       0  0 20:47 ?        00:00:00 /bin/sh -c /loop.sh
root           7       1 99 20:47 ?        00:00:04 bash /loop.sh
root           8       0  0 20:47 ?        00:00:00 ps -ef
```
Podemos observar como el proceso de ejecutar `/loop.sh` NO es directamente su proceso principal, esto se debe
a que primero debe crear una instancia de shel en `/bin/sh` para eventualmente poder correr shel.

Para modificar este comportamiento debemos cambiar el Docker file, especifica mente el último comando:

Cambiar de `shel form`
```doctest
CMD /loop.sh
```
A su `exec form`
...
CMD ["/loop.sh"]
```
Volvamos a construir la imagen y creemos el contenedor con el nuevo cambio del Dockerfile
```bash
docker rm -f looper
docker build -t loop .
docker run -d --name looper loop
...
```commandline
UID          PID    PPID  C STIME TTY          TIME CMD
root           1       0 95 20:54 ?        00:00:03 bash /loop.sh
root           7       0  0 20:54 ?        00:00:00 ps -ef
```
Ahora podemos observar que su `PID` 1 es el que esperaba ver `/loop.dh`
Entonces si detengo el contenedor:
```bash
docker stop looper
docker ps -l
```
...
```commandline
CONTAINER ID   IMAGE     COMMAND      CREATED              STATUS                          PORTS     NAMES
c5f094c793d0   loop      "/loop.sh"   About a minute ago   Exited (0) About a minute ago             looper
```
...
En docker podemos construir images a partir de un archivo llamado dockerfile
...
Si solo especificas un CMD, entonces Docker ejecutará dicho comando usando el ENTRYPOINT por defecto, que es `/bin/sh -c`.

Respecto al `entrypoint` (punto de entrada) y `cmd` (comando), puedes sobreescribir ninguno, ambos, o solo uno de ellos.
...
Al no contener el entrypoint toma /bin/ping y toma como parámetros los demás elementos `"-c", "3", "localhost"`; sin embargo, 
el argumento localhost es fijo por lo que no podemos hacer ping a otros destinos

El [Dockerfile](docker_platzi/docker/avanzado/ping/Dockerfile) original es el siguiente
...
Sin embargo, vamos a modificarlo para que pueda aceptar parámetros de entrada:
Vamos a modificarlo utilizando un `entrypoint` o un punto de entrada:
...
docker build -t ping .
```
Y vamos a construir nuestros contenedores de la siguiente manera:
```bash
...
Al igual que .gitignore en git, en Docker existe el .dockerignore que lista archivos o carpetas que se deben ignorar al 
momento de tomar en cuenta el contexto de build para construir una imagen.

Verse el archivo [.dockerignore](docker_platzi/docker/.dockerignore)
...
Al final el 2.º build es solo una extracción de lo que nos interesa del primer build.

Lo importante en este caso específico es que si el test falla, entonces el build 2 no se corre, lo que significa que la imagen no se construye.

Para hacer un build específico de un Dockerfile se usa el siguiente comando:

```bash
docker build <path_to_dockerfile> <context>
```

Ejemplo:
```bash
docker build -t app -f build/production.Dockerfile .
```
